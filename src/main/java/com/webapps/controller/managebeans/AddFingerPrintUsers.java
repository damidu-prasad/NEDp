 ...
 */
@ManagedBean(name = "AddFingerPrintUsers")
@ViewScoped
public class AddFingerPrintUsers {
    ...
    List<FingerPrintRegionUser> fingerPrintUsersList = new ArrayList<>();
...
//                        GoogleMail.Send("noreply.contex", "exoncontex1992", this.teacherList.get(i).getEmail(), subject, cont, null);
    ...
    public List<FingerPrintRegionUser> getFingerPrintUsersList() {
        return fingerPrintUsersList;
    }
    
    public void setFingerPrintUsersList(List<FingerPrintRegionUser> fingerPrintUsersList) {
        this.fingerPrintUsersList = fingerPrintUsersList;
    }
    ...
    public String updateTeacherRegistration() {
        ...
        boolean result = comDiv.updateTeacherRegistration(selectedTeacher.tid, this.firstName, this.lastName, this.nameWithIn, this.nic, this.dob, this.gender, this.mobile, this.email, this.address1, this.address2, this.address3,"","");
    ...
    public List<String> LoadTeacherSearch(String query) {
        ...
        String get_fingerPrintRegionUsers = "SELECT g FROM FingerPrintRegionUser g WHERE g.isActive='1' ORDER BY g.enrollementId ";
        
        List<FingerPrintRegionUser> fingerPrintRegionUsersList = uni.searchByQuery(get_fingerPrintRegionUsers);
        ...
        if (ls.getGeneralOrganizationProfileId().getOrganizationTypeId().getId() == 1) { // All
            ...
            for (Employee e : emp_list) {
                ...
                System.out.println("// Employee" + e.getGeneralUserProfileId().getNameWithIn());
                
                for (FingerPrintRegionUser fingerPrintRegionUser : fingerPrintRegionUsersList) {
                    
                    if (fingerPrintRegionUser.getGeneralUserProfileGupId().getId() == e.getGeneralUserProfileId().getId()) {
                        
                        System.out.println("// Employee2" + e.getGeneralUserProfileId().getNameWithIn());
    ...
    public void setSearchTeacherToTable(SelectEvent e) {
        
        fingerPrintUsersList.clear();
        ...
        if (!nic.equals("")) {
            ...
            if (emp_list.size() > 0) {
                ...
                String get_fingerPrintRegionUsers = "SELECT g FROM FingerPrintRegionUser g WHERE g.generalUserProfileGupId.id='" + emp.getGeneralUserProfileId().getId() + "' ORDER BY g.enrollementId ";
                
                List<FingerPrintRegionUser> fingerPrintRegionUsersList = uni.searchByQuery(get_fingerPrintRegionUsers);
                
                FingerPrintRegionUser fpru = fingerPrintRegionUsersList.iterator().next();
                
                fingerPrintUsersList.add(fpru);
                System.out.println("added teacher to list");
                searchTeacher = "";
            }
    ...
    public void viewFIngerPrintUser() {
        fingerPrintUsersList.clear();
        ...
        if (fingerPrintRegionUsersList.size() > 0) {
            ...
            for (FingerPrintRegionUser obj : fingerPrintRegionUsersList) {
                
                fingerPrintUsersList.add(obj);
                
            }
            
        } else {
            
            FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_WARN, "Finger Print Users Not Found", "");
            FacesContext.getCurrentInstance().addMessage(null, msg);
    ...
    public List<String> viewUnregisteredFingerPrintUsers(String query) {
        ...
        List<GeneralUserProfile> gup_list = new ArrayList<>();
        
        if (fingerPrintRegionUsersList.size() > 0) {
            
            for (FingerPrintRegionUser obj : fingerPrintRegionUsersList) {
                
                gup_list.add(obj.getGeneralUserProfileGupId());
        ...
        if (ls.getGeneralOrganizationProfileId().getOrganizationTypeId().getId() == 1) { // All
            ...
            List<Employee> emp_list = uni.searchByQuery("SELECT e from Employee e where ( e.generalUserProfileId.nic like '%" + query + "%' or e.generalUserProfileId.nameWithIn like '%" + query + "%') and  e.isActive='1' and e.generalOrganizationProfileId.id='" + loginUserSchoolId + "' order by e.generalUserProfileId.nameWithIn ASC");
            ...
            for (Employee e : emp_list) {
                ...
                for (FingerPrintRegionUser fingerPrintRegionUser : fingerPrintRegionUsersList) {
                    ...
                    if (!gup_list.contains(e.getGeneralUserProfileId())) {
    ...
    public void selectedTeacherSetToGupId(SelectEvent e) {
        System.out.println("value" + e.getObject().toString());
        
    }
    
    public void saveFingerPrintUser() {
        
        System.out.println("search Value " + searchValue + " Enrollement Value " + enrollementNo);
        
        int min = 0;
        int max = searchValue.indexOf("-");
        String nic = searchValue.substring(min, max);
        
        if (!searchValue.equals("") && !enrollementNo.equals("")) {
            
            String query_gup = "SELECT g FROM GeneralUserProfile g WHERE g.nic='" + nic + "' ";
            List<GeneralUserProfile> generalUserProfilesList = uni.searchByQuery(query_gup);
            System.out.println("A");
            if (generalUserProfilesList.size() > 0) {
                System.out.println("B");
                String query_fingerPrintUser = "SELECT g FROM FingerPrintRegionUser g WHERE g.enrollementId='" + enrollementNo + "' OR g.generalUserProfileGupId.id='" + generalUserProfilesList.iterator().next().getId() + "' ";
                List<FingerPrintRegionUser> fingerPrintRegionUsersList = uni.searchByQuery(query_fingerPrintUser);
                
                if (fingerPrintRegionUsersList.isEmpty()) {
                    
                    boolean result = comDiv.saveFingerPrintUser(generalUserProfilesList.iterator().next(), Integer.valueOf(enrollementNo));
                    ...
                    if (result == true) {
                        ...
                        FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_INFO, "Succesfully Saved !", "");
                        FacesContext.getCurrentInstance().addMessage(null, msg);
                        
                    } else {
                        
                        FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_WARN, "Finger Print User registration failed", "");
                        FacesContext.getCurrentInstance().addMessage(null, msg);
                    ...
                    FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_WARN, "The User or the Enrollement no that you are trying to add is already in the system !", "");
                    FacesContext.getCurrentInstance().addMessage(null, msg);
            ...
            FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_WARN, "The Teachers Name and the Enrollement id can not be empty", "");
            FacesContext.getCurrentInstance().addMessage(null, msg);
