...
import com.webapps.controller.utilities.SortArraysTeacherPerformanceAnalyzerOld;
 ...
 */
@ManagedBean(name = "teacherPerformanceAnalyzerOld")
@ViewScoped
public class TeacherPerformanceAnalyzerOld {
...
//            double mean = getMean(getProvinceSchoolList(0));
            double mean = stored.get_all_province_mean(Integer.parseInt(year), Integer.parseInt(termName));
            ...
            double mean_all_province = stored.get_all_province_mean(Integer.parseInt(year), Integer.parseInt(termName));

            double mean_selected_province = stored.get_selected_province_mean(Integer.parseInt(year), Integer.parseInt(termName), Integer.parseInt(provinceName));
            ...
            double mean_selected_zone = stored.get_selected_zone_mean(Integer.parseInt(year), Integer.parseInt(termName), Integer.parseInt(zoneName));
            ...
            // division
            double mean_selected_division = stored.get_selected_division_mean(Integer.parseInt(year), Integer.parseInt(termName), Integer.parseInt(divisionName));
            ...
            // school
            double mean_selected_school = stored.get_selected_school_mean(Integer.parseInt(year), Integer.parseInt(termName), Integer.parseInt(schoolName));
...
//                List<GradeClassSubjectTeacher> list_gcst = uni.searchByQuery("SELECT g FROM GradeClassSubjectTeacher g where g.gradeClassStreamManagerId.yearId.id='" + Integer.parseInt(year) + "' and g.gradeClassStreamManagerId.gradeClassStreamId.schoolId.id='" + school.getId() + "' and g.teacherId.id='" + gcst_individualy.getTeacherId().getId() + "' " + gradeQuery + subjectQuery + " ");
//                for (GradeClassSubjectTeacher gcst : list_gcst) { // me teacher me awurudde me iskole ugannapu panthi tika Ex : 3A,4A,4B
//
//                    List<GradeClassStudentsHasSubjects> list_gcshs = uni.searchByQuery("SELECT g FROM GradeClassStudentsHasSubjects g where g.gradeClassHasSubjectsId.id='" + gcst.getGradeClassHasSubjectsId().getId() + "' and g.gradeClassStudentsId.gradeClassStreamManagerId.id='" + gcst.getGradeClassStreamManagerId().getId() + "' and g.gradeClassStudentsId.isRemoved='0'");
//                    teacherTotStudents += list_gcshs.size();
//                    for (GradeClassStudentsHasSubjects gcshs : list_gcshs) {
//
//                        String querym = "SELECT g FROM StudentMarks g where g.gradeClassStudentsId.id='" + gcshs.getGradeClassStudentsId().getId() + "' and g.termsId.id='" + Integer.parseInt(termName) + "' and g.gradeClassHasSubjectsId.id='" + gcshs.getGradeClassHasSubjectsId().getId() + "'  and g.isRemoved='0'";
...
//
//                }
//                if (teacherTotStudents != 0) {
//                    meanTeacher = teacherTotMarks / teacherTotStudents;
//                }
...
//        }
        ...
        MeanTeacherList[] s = SortArraysTeacherPerformanceAnalyzerOld.GetArray(addressArray);
